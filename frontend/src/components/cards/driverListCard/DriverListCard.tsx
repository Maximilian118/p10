...
const DriverListCard: React.FC<driverListCardType> = ({ driver, onClick, canEdit, onEditClicked }) => {
  ...
  return (
    <div className="driver-list-card" onClick={onClick}>
      <div className="main-icon-container">
        <ImageIcon src={driver.url} size="contained"/>
        {canEdit && <EditButton
          onClick={e => {
            e.stopPropagation()
      ...
      <div className="driver-list-content">
        ...
        <div ref={driverTeamsRef} className="driver-list-teams">
          {driver.teams.map((team: teamType, i: number) => {
            if (i < lastIcon) {
              return <ImageIcon key={i} src={team.url}/>
            } else if (i === lastIcon) {
              return (
                <CounterIcon
                  key={i}
