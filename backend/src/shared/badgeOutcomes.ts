...
const badgeRewardOutcomes: badgeOutcomeType[] = [
  ...
  { name: "Q1 Casualty", awardedHow: "Q1 Elimination Bet", awardedDesc: "Bet on a driver who didn't make it out of Q1.", rarity: 0 },
  { name: "Q2 Casualty", awardedHow: "Q2 Elimination Bet", awardedDesc: "Bet on a driver who didn't make it out of Q2.", rarity: 0 },
  { name: "Euro Winner", awardedHow: "European Driver Win", awardedDesc: "Win with a European driver.", rarity: 0 },
  ...
  // New Uncommon badges.
  { name: "Grid Penalty", awardedHow: "Grid Penalty", awardedDesc: "Drop from position 1 to 5th or lower in one round.", rarity: 1 },
  { name: "Undercut", awardedHow: "Undercut", awardedDesc: "Pass 3+ competitors in standings in one round.", rarity: 1 },
  { name: "Points Penalty", awardedHow: "Points Penalty", awardedDesc: "Receive a negative points adjustment from the adjudicator.", rarity: 1 },
  ...
  { name: "Perfect P10", awardedHow: "Perfect P10", awardedDesc: "Bet on the exact P10 finisher.", rarity: 3 },
  { name: "Crowd Beater", awardedHow: "Large Field Win", awardedDesc: "Win a round with 10+ competitors.", rarity: 1 },
  ...
  // New Rare badges.
  { name: "Contrarian", awardedHow: "Contrarian", awardedDesc: "Win when you're the only one who bet on your driver.", rarity: 2 },
  { name: "Safety Car", awardedHow: "Safety Car", awardedDesc: "Score exactly 3 points in 3 consecutive rounds.", rarity: 2 },
  { name: "Track Limits", awardedHow: "Track Limits", awardedDesc: "Finish exactly P4 (just off podium) 3 times in a season.", rarity: 2 },
  ...
  { name: "Giant Slayer", awardedHow: "Underdog Win", awardedDesc: "Win with a driver from the lowest-ranked team.", rarity: 4 },
  ...
  { name: "Veteran Victory", awardedHow: "Over 35 Driver Win", awardedDesc: "Win with a driver older than 35.", rarity: 4 },
...
export default badgeRewardOutcomes
...
// Find the objects that contain the matching awardedHow value and return an array of outcomes.
export const findOutcomes = (
  badgeRewardOutcomes: badgeOutcomeType[],
  how: string,
): badgeOutcomeType[] => {
  return badgeRewardOutcomes.filter((outcome: badgeOutcomeType) => outcome.awardedHow === how)
}

// Find the objects that contain the matching awardedHow value and return an array of awardedHow's.
export const findHows = (badgeRewardOutcomes: badgeOutcomeType[], how: string): string[] => {
  return badgeRewardOutcomes
    .filter((outcome: badgeOutcomeType) => outcome.awardedHow === how)
    .map((outcome: badgeOutcomeType) => outcome.awardedHow)
}

// Find the objects that contain the matching awardedDesc value and return an array of awardedDesc's.
export const findDescs = (
  badgeRewardOutcomes: badgeOutcomeType[],
  desc: string | null,
): string[] => {
  return badgeRewardOutcomes
    .filter((outcome: badgeOutcomeType) => outcome.awardedDesc === desc)
    .map((outcome: badgeOutcomeType) => outcome.awardedDesc)
}

// Find the object that contains the awardedHow and return the relevant awardedDesc string.
export const findDesc = (badgeRewardOutcomes: badgeOutcomeType[], how: string | null): string => {
  const outcome = badgeRewardOutcomes.find(
    (outcome: badgeOutcomeType) => outcome.awardedHow === how,
  )
  return outcome?.awardedDesc ?? ""
}
